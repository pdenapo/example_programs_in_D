...
static class Web_Interface 
{
   ...
   @noAuth void index()
	...
	mustacheEngine.path  = "./views";
   	mustacheEngine.level = MustacheEngine!string.CacheLevel.no;
	mustacheContext["program_name"]= "Login example";
	mustacheContext["description"]="Example of a minimal login application using vibe-d";

	if (!user_authenticated)
		{
			string login_html=mustacheEngine.render("login", mustacheContext);
			response.writeBody(login_html,"text/html");
		}
	else 
		{
			string private_html=mustacheEngine.render("private", mustacheContext);
			response.writeBody(private_html,"text/html");
		}
	}
...
void main()
{
	...
	// sirve los archivos estáticos del directorio public_html
	router.get("*", serveStaticFiles("public_html/"));
